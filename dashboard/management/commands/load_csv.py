...
from dashboard.models import Repasses_Municipios, Despesas_Categoria_Economica, Despesas_favorecidos
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for row in DictReader(open('/home/khrir/Documents/Repositorios/Dashboard-AL/dashboard/databases/Repasses_M.csv')):
            rm_row = Repasses_Municipios(codigo=row['CODIGO'], total = row['TOTAL'], nome = row['NOME'])
            rm_row.save()

        for row in DictReader(open('/home/khrir/Documents/Repositorios/Dashboard-AL/dashboard/databases/despesas_CE.csv')):
            dce_row = Despesas_Categoria_Economica(custeio = row['CUSTEIO'], nome = row['DESCRICAO_UG'])
            dce_row.save()
        
        for row in DictReader(open('/home/khrir/Documents/Repositorios/Dashboard-AL/dashboard/databases/despesas_PF.csv')):
